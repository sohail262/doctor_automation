...
exports.generateWhatsAppResponse = generateWhatsAppResponse;
const openai_1 = __importDefault(require("openai"));
const functions = __importStar(require("firebase-functions"));
const openai = new openai_1.default({
    apiKey: functions.config().openai?.key || process.env.OPENAI_API_KEY,
});
async function generatePostContent(doctorName, specialty, topic, platform = 'google_my_business') {
...
async function generateReviewReply(doctorName, specialty, reviewerName, rating, reviewText, knowledgeBase) {
...
async function parseAppointmentRequest(message, doctorName) {
...
async function generateWhatsAppResponse(context, doctorName, specialty) {
